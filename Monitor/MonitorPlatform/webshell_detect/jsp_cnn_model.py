...
max_features = 448  #
max_document_length = 192  # The opcode/bytecode length of the sample file
min_code_count = 0  # The min opcode/bytecode length of sample file

JspCnnModel = "webshell_detect_cnn_model/jsp_cnn_model/jsp-webshell-detect-cnn-model"
...
def jsp_cnn_model():
    ...
    branch1 = conv_1d(network, 128, 3, padding='valid', activation='relu', regularizer="L2")  # network filters CNN-core
    branch2 = conv_1d(network, 128, 4, padding='valid', activation='relu', regularizer="L2")
    branch3 = conv_1d(network, 128, 5, padding='valid', activation='relu', regularizer="L2")
    branch4 = conv_1d(network, 128, 6, padding='valid', activation='relu', regularizer="L2")  # network filters CNN-core
    branch5 = conv_1d(network, 128, 7, padding='valid', activation='relu', regularizer="L2")
    branch6 = conv_1d(network, 128, 8, padding='valid', activation='relu', regularizer="L2")
    network = merge([branch1, branch2, branch3, branch4, branch5, branch6], mode='concat', axis=1)  # 融合层
...
def get_jsp_cnn_model(train_opcode, train_tags):
    ...
    model.fit(train_opcode, train_tags, n_epoch=40, shuffle=True, validation_set=0.1, show_metric=True, batch_size=20,
              run_id="jsp-webshell-datect-cnn")  # 训练迭代次数5  shuffle布尔值是否在每轮迭代之前混洗数据   作验证集的训练数据的比例0.1-模型将分出一部分不会被训练的验证数据，并将在每一轮结束时评估这些验证数据的误差和任何其他模型指标   batch_size-每次梯度更新的样本数
    print("CNN training is over!!!")
    model.save(JspCnnModel)


# test CNN model
def test_jsp_cnn_model(test_opcode, test_tags):
    print("Start test CNN model:")
    with tf.Graph().as_default():
        new_model = jsp_cnn_model()
        new_model.load(JspCnnModel)
        model1 = new_model
