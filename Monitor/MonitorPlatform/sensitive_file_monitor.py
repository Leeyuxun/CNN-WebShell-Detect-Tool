...
class MyFileEventHandler(pyinotify.ProcessEvent):
    ...
    def process_IN_MODIFY(self, event):
        ...
        with open('MonitorPlatform/sensitive_file_list.txt', 'r') as f:
            for line in f:
                sensitive_file_list.extend(line.strip('\n').split(','))
        ...
        for i in sensitive_file_list:
            if i in event.pathname and (i+'~') not in event.pathname:
                ...
                sensitiveFileMonitorLog.objects.create(
                    sensitiveFilename=event.pathname,
                    event="编辑文件"
                )
...
def sens_file_monitor():
    sensitive_file_list = []
    sensitive_path_list_1 = []
    with open('MonitorPlatform/sensitive_file_list.txt', 'r') as f:
        for line in f:
            sensitive_file_list.extend(line.strip('\n').split(','))
    ...
    for i in sensitive_file_list:
        tmp = i.rindex('/')
        sensitive_path = i[:tmp + 1]
        sensitive_path_list_1.append(sensitive_path)

    sensitive_path_list = list(set(sensitive_path_list_1))
